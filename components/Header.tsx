    ...
    <header className="sticky top-0 z-50 bg-white/85 backdrop-blur-md border-b border-[#2D5A27]/10 shadow-sm">
      <div className="max-w-6xl mx-auto px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          {/* Auth Buttons - Rounded full */}
          <div className="hidden lg:flex items-center gap-3">
            <Link href="/connexion" className="px-5 py-2 text-sm text-[#2D5A27]/80 hover:text-[#2D5A27] border border-[#2D5A27]/20 hover:border-[#2D5A27]/40 rounded-full transition-all">
              Connexion
            </Link>
            <Link href="/inscription" className="px-5 py-2 text-sm bg-[#5B9A4A] hover:bg-[#4A8A3A] text-white font-semibold rounded-full transition-all shadow-lg shadow-[#5B9A4A]/20">
              S&apos;inscrire
            </Link>
          </div>
        ...
        {isMenuOpen && (
          <nav className="lg:hidden mt-4 pt-4 border-t border-[#2D5A27]/10">
            ...
            <div className="flex gap-3 mt-4 pt-4 border-t border-[#2D5A27]/10">
              <Link href="/connexion" className="flex-1 py-3 text-center text-sm text-[#2D5A27] border border-[#2D5A27]/20 rounded-full">Connexion</Link>
              <Link href="/inscription" className="flex-1 py-3 text-center text-sm bg-[#5B9A4A] text-white font-semibold rounded-full">S&apos;inscrire</Link>
            </div>
          </nav>
        )}
      </div>
